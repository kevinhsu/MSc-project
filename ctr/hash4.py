...
submission = 'submission44.csv'  # path of to be outputted submission file
...
holdafter = 200   # data after date N (exclusive) are used as validation
...
# A. Subsampling training set
def subsampling(train, D):
    holdout = 40    # One million samples
...
for e in xrange(epoch):
    ...
    for t, date, ID, x, y in data(train, D):  # data is a generator
        ...
        if (holdafter and date > holdafter) or (holdout and t % holdout == 0):
            ...
            #         datetime.now(), count, (loss * 1./count)))
            print "===="
        else:
